...
public class OAuth2Configuration extends AuthorizationServerConfigurerAdapter {
	...
	public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
		clients.inMemory().withClient("travel-client").secret("travel-secret")
				.authorities(OAuth2Authority.ROLE_ADMIN.name(), OAuth2Authority.ROLE_USER.name())
				.authorizedGrantTypes("refresh_token", "password").accessTokenValiditySeconds(ONE_DAY)
	...
	public void configure(AuthorizationServerSecurityConfigurer oauthServer) throws Exception {
		...
		oauthServer.tokenKeyAccess(
				"isAnonymous() || hasAuthority('" + OAuth2Authority.ROLE_ADMIN + "," + OAuth2Authority.ROLE_USER + "')")
